 ...
 *
 */
public class JDBCOperation implements InterfaceLoginCheck {
	...
	public void insert(Connection conn, User user) throws SQLException {
		String saveSql="INSERT INTO tbl_user(name, password, email, sex, signUpTime) VALUES(?,?,?,?,?)";
	...
	public ResultSet getName(Connection conn, User user) throws SQLException {
		String resultSetSql="SELECT * From tbl_user WHERE name=?";
		PreparedStatement ps=conn.prepareStatement(resultSetSql);
		ps.setString(1, user.getName());
		System.out.println("======== 调用了JDBCLoginCheck的getName()方法 ========");
		return ps.executeQuery();
	}
}
